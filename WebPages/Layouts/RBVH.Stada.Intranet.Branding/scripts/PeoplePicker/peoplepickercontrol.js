...
(function (CAMControl) {
    var PeoplePicker = (function () {
        ...
        function PeoplePicker(SharePointContext, PeoplePickerControl, PeoplePickerEdit, PeoplePickerDisplay, PeoplePickerData, ServerDataMethod, LimitToGroupName) {
            ...
            this.SpHostUrl = decodeURIComponent(RBVH.Stada.WebPages.Utilities.GetValueByParamURL('SPHostUrl'));
            this.InstanceName = "";
        ...
        PeoplePicker.prototype.GetFilterData = function () {
        ...
        // Property wrapped in function to allow access from event handler
        PeoplePicker.prototype.SetFilterData = function (FilterData) {
            //See http://msdn.microsoft.com/en-us/library/office/microsoft.sharepoint.client.utilities.principaltype.aspx
            //This enumeration has a FlagsAttribute attribute that allows a bitwise combination of its member values.
            //None Enumeration whose value specifies no principal type. Value = 0. 
            //User Enumeration whose value specifies a user as the principal type. Value = 1. 
        ...
        PeoplePicker.prototype.ShowSelectionBox = function () {
            ...
            this.PeoplePickerDisplay.css('max-height', '200px');
            this.PeoplePickerDisplay.css('overflow-y', 'scroll');
        };
        ...
        PeoplePicker.prototype.ConstructResolvedUserSpan = function (login, name, lookupId) {
            ...
            var userDisplaySpanTemplate = '<p style="margin: 0 0"><span class="cam-peoplepicker-userSpan"><span class="cam-entity-resolved">{0}</span><a title="{3}" class="cam-peoplepicker-delImage" onclick="{1}.DeleteProcessedUser({2}); return false;" href="#">x</a></span></p>';

            return this.Format(userDisplaySpanTemplate, name, this.InstanceName, "'" + lookupValue + "'", resultDisplay);
        };

        // resize html box based on number of people
        PeoplePicker.prototype.resizeBox = function () {
            if (this._ResolvedUsers.length < 1) {
                $("#" + this.textBoxId).css("height", "30px");
            } else {
                $("#" + this.textBoxId).css("height", 30 + this._ResolvedUsers.length * 20 + "px");
            }
        }
        ...
        PeoplePicker.prototype.PushResolvedUser = function (resolvedUser) {
            ...
            this.PeoplePickerData.trigger('change');
            this.resizeBox();
        };
            ...
            if (results && results.length) {
                if (results.length > 0) {
                    ...
                    for (var i = 0; i < displayCount; i++) {
                        ...
                        txtResults += this.Format(displayTemplate, this.InstanceName, loginName.replace("\\", "\\\\"), this.HtmlEncode(displayName), email, displayName, loginNameDisplay);
                    }
                    var resultDisplay = '';
        ...
        PeoplePicker.prototype.Initialize = function () {
            ...
            if (this.PeoplePickerData.val() != null && this.PeoplePickerData.val().length > 0) {
                    ...
                    if (txt.length > 0) {
                        ...
                        if (searchText.length >= parent.GetMinimalCharactersBeforeSearching()) {
                            ...
                            if (typeof parent.ServerDataMethod === 'undefined') {
                                ...
                                var PrincipleType = parent.GetPrincipalType();

                                if (PrincipleType == 1) {
                                    var dataRequest = {
                                        Name: searchText,
                                        EmployeePositions: parent.GetFilterData()
                                    }
                                    $.ajax({
                                        type: "POST",
                                        url: parent.GetServerDataMethod(),
                                ...
                                else if (PrincipleType == 2) {
                                    $.ajax({
                                        type: "GET",
                                        url: parent.GetServerDataMethod() + "/" + encodeURIComponent(searchText),
                                        contentType: "application/json; charset=utf-8",
                                        dataType: "json",
                                        success: function (msg) {
                                            parent.QuerySuccess(queryIDToPass, msg);
