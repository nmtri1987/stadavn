...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="LeaveRequestControl.ascx.cs" Inherits="RBVH.Stada.Intranet.WebPages.CONTROLTEMPLATES.RBVH.Stada.Controls.LeaveManagementControl.LeaveRequestControl" %>

<div style="margin-bottom: 20px; margin-left: -5px">
    <button type="button" id="btnAddNewLeave" class="btn btn-primary add-new-leave">
        <i class='fa fa-plus' aria-hidden='true'></i>&nbsp;
        <asp:Literal runat="server" Text="<%$Resources:RBVHStadaWebpages,LeaveManagement_AddNew%>" />
    </button>
</div>
<table style="table-layout: fixed; width: 100% !important" id="leave-request-list-container">
    <tr>
        <td style="width: 100%;" valign="top">
            <asp:HiddenField ID="ParamRequesterLookupIDHidden" runat="server"></asp:HiddenField>
            <br />
            <WebPartPages:WebPartZone runat="server" FrameType="None" ID="MyLeaveRequestZone" Title="loc:Main">
            <ZoneTemplate>
                ...
                __AllowXSLTEditing="true" ID="LeaveRequestWebPart" WebPart="true" Height="" Width="">
                <ParameterBindings>
                ...
                <JSLink>~sitecollection/_layouts/15/RBVH.Stada.Intranet.Branding/scripts/LeaveModule/JSLink_LeaveRequest_List.js?v=1.1</JSLink>
                </WebPartPages:XsltListViewWebPart>
            </ZoneTemplate>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('.add-new-leave').on('click', function () {
            var sourceURL = window.location.href;
            sourceURL = Functions.removeParam('lang', sourceURL);
            sourceURL = encodeURIComponent(sourceURL);
            window.location = '//' + location.host + '/SitePages/LeaveRequest.aspx?subSection=LeaveManagement&Source=' + sourceURL;
        });
    });
</script>
