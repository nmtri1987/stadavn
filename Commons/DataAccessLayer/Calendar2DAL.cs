...
{
    public class Calendar2DAL : BaseDAL<Calendar>
    {
        public Calendar2DAL(string siteUrl) : base(siteUrl)
        {
            ListUrl = "/Lists/CompanyCalendar2";
        }
        public override Calendar ParseToEntity(SPListItem listItem)
        {
            ...
            location = string.IsNullOrEmpty(location) ? StringConstant.FactoryLocation2 : location;
        ...
        //    var calendarItems = this.GetByQueryToSPListItemCollection(queryStr);
                                    ...
                                    <And>
                                        <In>
                                            <FieldRef Name='Category'/>
                                                <Values>{categoriesString}</Values>
                                        </In>
                                        <And>
                                            <Leq>
                                                <FieldRef Name='EventDate' />
                                                <Value IncludeTimeValue='TRUE' Type='DateTime'>{toDateString}</Value>
                                            </Leq>
                                            <Geq>
                                                <FieldRef Name='EndDate' />
                                                <Value IncludeTimeValue='TRUE' Type='DateTime'>{fromDateString}</Value>
                                            </Geq>
                                        </And>
                                    </And>
                                </Where>";
